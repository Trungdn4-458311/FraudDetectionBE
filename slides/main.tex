...
\subtitle{Business Analytics Group Project --- Modules 1--5}
...
\begin{enumerate}
    ...
    \item Model results \hfill \textit{(M5)}
    \item Cost-based threshold \hfill \textit{(M5)}
...
\begin{itemize}
    \item \textbf{9 contextual features}
...
\textbf{RF $\cdot$ Base}  & \good{0.9974} & 0.9967 & 0.9996 & \good{1} & 8 \\
RF $\cdot$ Full           & 0.9996 & 0.9963 & 1.0000 & 0 & 9 \\
XGBoost $\cdot$ Base      & 0.9944 & 0.9923 & 0.9204 & 211 & 19 \\
XGBoost $\cdot$ Full      & 0.9997 & 0.9976 & 0.9927 & 18 & 6 \\
\bottomrule
\end{tabular}
\end{table}
...
Random Forest       & 0.9974 & 0.9996 & \good{$+0.0022$} \\
XGBoost             & 0.9944 & 0.9997 & \good{$+0.0053$} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\alert{Optimal $t^{*} = 0.21$, not $0.5$ $\Rightarrow$ total cost $-16.6\%$}
\end{frame}
...
\good{False-positive rate} & \good{$0.031\%$} (260 transactions) \\
\bottomrule
\end{tabular}
\end{table}
...
\vspace{0.5em}
\centering
\textbf{Next:} deployment (M6), monitoring (M7), risk-policy recommendations (M8)
\end{frame}
...
\begin{itemize}
    \item Ranking gap is \emph{small}: PR-AUC $0.9974$ vs $0.9944$
    \item Precision gap is \emph{large}: $0.9996$ vs $0.9204$ (1 vs 211 FP)
