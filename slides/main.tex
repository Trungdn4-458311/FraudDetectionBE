...
\author[Group]{\small Do Ngoc Trung {\footnotesize(20232311M)} \quad Vu Quy Dat {\footnotesize(20242432M)}\\[0.2em]
Le Xuan Nam {\footnotesize(20241012M)} \quad Do Hoang Dung {\footnotesize(20241184M)}}
\institute[]{Supervisor: Ph.D Bui Quoc Trung \\[0.3em] Business Analytics}
\date{}
...
\vspace{1em}
\begin{itemize}
    \item \texttt{errorBalanceOrig} alone $= \good{61.8\%}$
    \item Synthetic total $\approx 8\%$
...
\begin{itemize}
    ...
    \item \texttt{errorBalanceOrig} captures it directly ($61.8\%$ importance)
...
\end{itemize}
\vspace{0.4em}
Free-tier deploy (Streamlit Community Cloud), one-command push.
\end{column}
\end{columns}
\vspace{0.5em}
\centering
...
\vspace{0.8em}
\begin{itemize}
    \item $t^{*} \approx 0.09$ (deployed model, cost-optimal); \good{$b \approx 0.80$} --- target precision $\ge 0.99$, else conservative fallback
...
\vspace{2em}
\normalsize
\url{https://github.com/namxle-hust/FraudDetectionBE}
\end{frame}
